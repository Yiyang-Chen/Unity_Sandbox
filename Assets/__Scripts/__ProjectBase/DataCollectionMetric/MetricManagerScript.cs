...
public class MetricManagerScript : MonoBehaviour
{
    ...
    //timers 
    [SerializeField] [Tooltip("How many seconds between collecting timed variables")]
    float _intervalBetweenTimedVariables = 5.0f;

    [SerializeField] [Tooltip("How many seconds between writing to the file")]
    float _intervalBetweenWritingToFile = 60.0f;

    [SerializeField] [Tooltip("Collect data on Editor?")]
    ...
    void Start()
    {
        if (ShouldCollectData) //Collect on build, only collect on Editor if _collectDataOnEditor is true. This is to avoid trash files
        {
            if (instance == null)
            {
                ...
                _dataToWriteToFile.Append(
                    "Variable name,Time in Play,Data Point"); //separate the title for each collumn with a comma
                ...
                _fileName = "Metrics_" + _timeStamp + "_" + tempDevice + ".csv";
                string pathName = "./";
                ...
                {
                    MinimalEnvironment.Instance.GetSystem<LogSystem>().Warn("Could not create telemetry file " + pathName + _fileName);
    ...
    public void LogString(string variableName, string dataToLog)
    {
        ...
        string dataLine = variableName + "," + minutesDisplay.ToString() + ":" + secondsDisplay.ToString("F2") + "," +
                          dataToLog.ToString() + Environment.NewLine;
